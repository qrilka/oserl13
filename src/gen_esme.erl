...
%% @spec swap_disk_log(ServerRef, Args) -> ok
%%    ServerRef = Name | {Name, Node} | {global, Name} | pid()
%%    Name = atom()
%%    Node = atom()
%%    Args = term()
...
%%    BinaryPdu = binary()
%%
%% @doc Swaps the ESME disk log.  Refer to <a href="smpp_log.html#swap_disk_log_handler-1">smpp_log:add_disk_log_handler/1</a> to see permitted <tt>Args</tt>.
%%
%% <p><tt>Pred</tt> is a fun <tt>fun(BinaryPdu) -&gt; bool()</tt></p>
%%
%% @see smpp_log:add_disk_log_handler/1
%% @end 
swap_disk_log(ServerRef, Args) ->
    gen_server:cast(ServerRef, {swap_disk_log, Args}).

%% @spec swap_error_logger(ServerRef, Args) -> ok
%%    ServerRef = Name | {Name, Node} | {global, Name} | pid()
%%    Name = atom()
%%    Node = atom()
%%    Args = term()
...
%%    BinaryPdu = binary()
%%
%% @doc Swaps the ESME error logger.  Refer to <a href="smpp_log.html#add_error_logger_handler-1">smpp_log:add_error_logger_handler/1</a> to see permitted <tt>Args</tt>.
%%
%% <p><tt>Pred</tt> is a fun <tt>fun(BinaryPdu) -&gt; bool()</tt></p>
%%
%% @see smpp_log:add_error_logger_handler/1
%% @end 
swap_error_logger(ServerRef, Args) ->
    gen_server:cast(ServerRef, {swap_error_logger, Args}).
...
handle_cast({open_error_logger, Args}, S) ->
    ...
    {noreply, S};
handle_cast({swap_disk_log, Args}, S) ->
    smpp_log:swap_disk_log_handler(S#state.log, Args),
    {noreply, S};
handle_cast({swap_error_logger, Args}, S) ->
    smpp_log:swap_error_logger_handler(S#state.log, Args),
    {noreply, S}.
