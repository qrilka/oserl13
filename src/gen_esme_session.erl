%%% Copyright (C) 2004 - 2005 Enrique Marcote Pe�a <mpquique@users.sourceforge.net>
%%%
...
%%% </ul>
%%%
%%% @copyright 2004 - 2005 Enrique Marcote Pe�a
...
reply_async([{uid,Uid,_}|_], From) ->
    gen_fsm:reply(From, {accepted, Uid});
reply_async(_ParamList, _From) ->
    ok.
                ...
                {[[RTimer, From]], _Continuation} -> % Expected response
                    ...
                    case operation:get_param(command_status, Pdu) of
                        ?ESME_ROK when
                            ...
                            gen_fsm:reply(From, {ok, Pdu});
                        ?ESME_ROK when CmdId == ?COMMAND_ID_SUBMIT_SM_RESP ->
                            case From of
                          {_RealFrom,{uid,Uid,{M,F}}} ->
                              %gen_fsm:reply(RealFrom, {ok, {{uid,Uid},Pdu}});
                              Id = operation:get_param(message_id, Pdu),
                              apply(M,F,[{ok, {async_submit,{uid,Uid},{id,Id}}}]);
                          _ ->
                              gen_fsm:reply(From, {ok, Pdu})
                            end;
                        ?ESME_ROK when CmdId == ?COMMAND_ID_QUERY_SM_RESP ->
                            %%io:format("QUERY_SM_RESP, From=~p~n", [From]),
                            case From of
                          {_RealFrom,{uid,Uid,{M,F}}} ->
                              FinalDate = operation:get_param(final_date, Pdu),
                              MsgState = operation:get_param(message_state, Pdu),
                              apply(M,F,[{ok, {async_query,{uid,Uid},
                                                           {final_date,FinalDate},
                                                           {state,MsgState}}}]);
                              %%io:format("RcA=~p~n",[RcA]);
                          _ ->
                              gen_fsm:reply(From, {ok, Pdu})
                            end;
                        ?ESME_ROK ->
                            gen_fsm:reply(From, {ok, Pdu});
                        Status when CmdId == ?COMMAND_ID_SUBMIT_SM_RESP ->
                            case From of
                          {_RealFrom,{uid,Uid,{M,F}}} ->
                              apply(M,F,[{error, {async_submit,{uid,Uid},Status}}]);
                          _ ->
                              gen_fsm:reply(From, {error, Status})
                            end;
                        Status when CmdId == ?COMMAND_ID_QUERY_SM_RESP ->
                            case From of
                          {_RealFrom,{uid,Uid,{M,F}}} ->
                              apply(M,F,[{error, {async_query,{uid,Uid},Status}}]);
                          _ ->
                              gen_fsm:reply(From, {error, Status})
                            end;
                        Status ->
...
%% @see send_request/3
%% @end
send_request(CmdId, [{uid,Uid,MF}|ParamList], From, S)
  when CmdId == ?COMMAND_ID_SUBMIT_SM;
       CmdId == ?COMMAND_ID_QUERY_SM ->
    send_request2(CmdId, ParamList, {From,{uid,Uid,MF}}, S);

send_request(CmdId, ParamList, From, S) ->
  send_request2(CmdId, ParamList, From, S).
...
%%		io:format("time before send_pdu: ~p~n", [now()]),
    send_pdu(S#state.socket,operation:new(CmdId,SeqNum,ParamList),S#state.log),
%%		io:format("time after send_pdu: ~p~n", [now()]),
