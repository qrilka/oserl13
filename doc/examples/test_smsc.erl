...
handle_bind({bind_transceiver, Trx, _Pdu, _IpAddr}, _From, S) ->
    ParamList = [{system_id, ?SYSTEM_ID},{sc_interface_version, 16#50}],
%    proc_lib:spawn_link(fun() -> gen_smsc:session_stop(Trx) end),
    {reply, {error, ?ESME_RINVSYSID, ParamList}, S}.
...
handle_operation({CmdName, _Session, Pdu}, _From, S) when CmdName == submit_sm;
    ...
    Ton = operation:get_param(dest_addr_ton, Pdu),
    Npi = operation:get_param(dest_addr_npi, Pdu),
    io:format("~p: ~p - ~p - ~p: ~p~n", [CmdName, N, Ton, Npi, T]),
